...
router.get('/submitdingdan',async (request,response)=>{
    ...
    if(_id !== '0'){
        ...
        var aaa = [zongshuliang, zongjia]
        response.render('dingdanmes',{jieguo,aaa,xiaoji})
    }else{
        response.redirect('login')
    }
})
...
router.get('/Mydingdan',async (request,response)=>{
    ...
    if(_id !== '0'){
        ...
        // var yiyongjifen = 0
        // for(var i = 0 ;i<aa.dingdan.length ; i++){
        //     yiyongjifen = Number(aa.dingdan[i].jifen)
        // }

        await userModel.updateOne({_id: _id}, {$set: {gouwuche: []}}, function (err, data) {
            console.log(data, '**清空购物车')
        })
        // await userModel.updateOne({_id: _id}, {$set: {jifen: Number(shengyujifen)}}, function (err, data) {
        //         //     console.log(data, '**积分剩余')
        //         // })
        let jieguo = await userModel.findOne({_id:_id})
